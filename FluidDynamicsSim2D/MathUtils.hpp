...
vector<vector<unique_ptr<int>>> normalize2DMatrix(vector<vector<unique_ptr<float>>>& in, float inMin, float inMax, float outMin, float outMax) {    //Pass simulationdata as reference, as the only way otherwise would be to move the data in which would erase it from original memory
    ...
    for(int dim2i = 0; dim2i<in.size(); dim2i++) {
        ...
        for(int dim1i = 0; dim1i<in.at(dim2i).size(); dim1i++) {
            ...
            * efficient one.
            */
            unique_ptr<int> dim1 = make_unique<int>(round((oldVal*(outMax-outMin))/(inMax-inMin)));
